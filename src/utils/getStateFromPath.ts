...
function matchAgainstConfigs(
  ...
  for (const config of configs) {
    ...
    const routeFromName = (name: string) => {
      const configByName = configs.find((c) => c.screen === name);
      if (!configByName?.path) {
        return { name };
      }

      const segments = configByName.path.split('/');
      ...
      segments
        ...
        .forEach((p) => {
          ...
          if (value) {
            ...
            params[key] = configByName.parse?.[key]
              ? configByName.parse[key]!(value)
              : value;
          }
        });
